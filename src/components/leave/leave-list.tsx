...
export function LeaveList({ requests, isManager }: LeaveListProps) {
  ...
  const handleBulkReject = async () => {
    ...
    const reason = window.prompt(
      `Reject ${selectedIds.size} leave request${selectedIds.size === 1 ? "" : "s"}. Optional rejection reason:`,
      "",
    );
    if (reason === null) return; // user cancelled
    setBulkRejecting(true);
    try {
      const res = await fetch(`/api/leave/bulk-reject`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      const data = await res.json();
      toast({
        title: `Rejected ${data.rejected.length} request${data.rejected.length === 1 ? "" : "s"}`,
        description: data.skipped.length
          ? `${data.skipped.length} skipped (already actioned or invalid).`
          : "All selected requests rejected.",
      });
      setSelectedIds(new Set());
      router.refresh();
    } catch (err) {
