...
import { getQBCreds, getQBAccessToken, qboBase, QBO_MINOR_VERSION } from "@/lib/qb-api";
  ...
  try {
    ...
    switch (action) {
      case "query": {
        ...
        url = `${base}/query?query=${encodeURIComponent(q)}&minorversion=${QBO_MINOR_VERSION}`;
        break;
      }
      case "read": {
        if (!id) return NextResponse.json({ success: false, error: "id required for read" }, { status: 400 });
        url = `${base}/${entity}/${id}?minorversion=${QBO_MINOR_VERSION}`;
        break;
      }
      case "create": {
        url = `${base}/${entity}?minorversion=${QBO_MINOR_VERSION}`;
        method = "POST";
      ...
      case "delete": {
        url = `${base}/${entity}?operation=delete&minorversion=${QBO_MINOR_VERSION}`;
        method = "POST";
        body = JSON.stringify(reqBody);
        break;
      }
      case "void": {
        url = `${base}/${entity}?operation=void&minorversion=${QBO_MINOR_VERSION}`;
        method = "POST";
        body = JSON.stringify(reqBody);
        break;
      }
      case "send": {
        ...
        const sendQuery = sendTo ? `?sendTo=${encodeURIComponent(sendTo)}&minorversion=${QBO_MINOR_VERSION}` : `?minorversion=${QBO_MINOR_VERSION}`;
      ...
      case "pdf": {
        ...
        url = `${base}/${entity}/${id}/pdf?minorversion=${QBO_MINOR_VERSION}`;
