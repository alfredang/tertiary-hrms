...
## Session 5: App Store Screenshots & Listing Metadata

- [x] Created Playwright screenshot script (`scripts/take-app-store-screenshots.ts`)
  - 8 screenshots: login, admin dashboard, leave admin, payroll, staff dashboard, leave staff, expenses, calendar
  - Captures against production (`hrms.tertiaryinfo.tech`)
  - Two variants: `screenshots/raw/` (clean) and `screenshots/with-headers/` (gradient banner + marketing text)
  - Resolution: 1290×2796px (iPhone 14 Pro Max, App Store 6.7" display)
  - Composite approach: raw screenshot + separate HTML page with solid gradient header (no injection into live app)
- [x] Created App Store listing metadata (`screenshots/APP_STORE_LISTING.md`)
  - App name, subtitle, full description, promotional text
  - Keywords (100 chars), category (Business/Productivity)
  - Age rating (4+), privacy policy URL, support URL
  - Test accounts for Apple/Google review (admin + staff)
  - Screenshot upload order recommendation
  - Google Play equivalents (short description, content rating)
- [x] Removed employees screenshot (contains real employee data)
- [x] Removed settings screenshot (user request)
- Iterated on header design: solid padding → transparent overlay → final composite approach
...
1. **Push** security + test commits to deploy to Coolify (3 commits ahead)
2. **Commit** App Store screenshots + listing metadata + screenshot script
3. Run `npm audit fix` for non-breaking dependency patches
4. Consider `next` and `next-auth` version bumps (test carefully)
5. If boss provides API keys → configure UploadThing / Resend / Chat
6. Re-run screenshot script after any UI changes: `npx tsx scripts/take-app-store-screenshots.ts`
...
### Session 5 (uncommitted — App Store assets)
- `scripts/take-app-store-screenshots.ts` — New, Playwright screenshot generator
- `screenshots/raw/*.png` — 8 raw screenshots (1290×2796)
- `screenshots/with-headers/*.png` — 8 screenshots with gradient marketing headers
- `screenshots/APP_STORE_LISTING.md` — Copy-paste-ready App Store & Google Play listing
